...
**ETag / `304 Not Modified`** (2026-09-02) — la réponse porte désormais un header `ETag`. Renvoyer sa valeur en `If-None-Match` sur la requête suivante (même page, mêmes filtres) déclenche un `304` sans corps si rien n'a changé — utile pour un retour en arrière de pagination ou un auto-refresh périodique. ⚠️ Ce n'est **pas** un cache "produit inchangé depuis longtemps" au sens strict : `stock_available` et `price` (prix partenaire) sont des données vivantes qui font varier l'ETag dès qu'elles bougent, même si le produit lui-même n'a pas été modifié — donc pas de faux `304` avec un stock ou un prix périmé, mais aussi pas d'économie de requête côté serveur (la requête tourne toujours ; seul l'encodage JSON + le transfert réseau sont évités sur un `304`).
...
**`GET /partners` — ETag / `304 Not Modified`** (2026-09-02) — même mécanisme qu'en §4.1 : la réponse porte un header `ETag`, un `If-None-Match` correspondant sur la requête suivante renvoie un `304` sans corps. `credit_used`/`credit_available` sont vivants (varient à chaque commande/paiement, même sans que la fiche partenaire elle-même soit modifiée) et sont inclus dans le calcul de l'ETag — pas de `304` avec un encours périmé.
...
- **2026-09-02 (ETag / `304 Not Modified`)** : `GET /catalog/products` (§4.1) et `GET /partners` (§5) portent désormais un header `ETag` — un `If-None-Match` correspondant renvoie un `304` sans corps. Demande initiale de l'équipe UI : `hash(max(updated_at) + count)` calculé **avant** la requête, pour sauter la requête entièrement sur un match. Écarté après analyse : les deux réponses embarquent des champs vivants (`stock_available`, prix partenaire résolu, `credit_used`/`credit_available`) qui ne font pas forcément bouger `updated_at` de la ligne sous-jacente — un hash pré-requête aurait pu servir un `304` avec un stock ou un encours périmé, exactement le genre d'erreur silencieuse dangereuse pour un canal de vente en temps réel. La requête tourne donc toujours ; l'ETag est calculé sur la réponse déjà résolue — économie réelle sur l'encodage JSON + le transfert réseau pour une page inchangée (pagination aller-retour, auto-refresh), aucun risque de donnée périmée.
