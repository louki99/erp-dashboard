...
File:
- `docs/Dispatcher_Backend_API.postman_collection.json`
...
2. Set collection variables:
   - `BASE_URL`
   - `TOKEN`
   - `ORDER_ID`, `BL_ID`, `BCH_ID`, `DECHARGE_ID`, `LIVREUR_ID`
3. Call `Auth (Backend) > Login (get token)` then copy the returned token into `TOKEN`.
...
Important:
- Always send `Accept: application/json` + a valid `Authorization: Bearer {{TOKEN}}`.
- If you call an API route without these headers or without auth, Laravel may respond with HTML (login page / 404 page).
...
Response:
```json
{ "success": true, "data": { "draftBls": [], "livreurs": [] } }
```
